	...
	for(l= &t->l[0], i=0; i<NL; i++, l++){
        ...
        Again:
		 	if(l->f.nlines==l->f.maxlines && /* BUG: lastlinefull!! */
		   		(p=ptofchar(&l->f, l->f.nchars),p.y)>=
				  l->f.r.origin.y+l->f.maxlines*fheight(l->f.font))
