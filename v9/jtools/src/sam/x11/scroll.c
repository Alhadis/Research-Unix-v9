	...
	register Flayer *l;
{
 	...
 	while(button(but)){
		...
		if(in){
			...
			if(my>=s.corner.y)
				my=s.corner.y;
			if(my != mouse.xy.y)
				cursset(Pt(mouse.xy.x, my));
			if(but==1){
				p0=l->origin-charofpt(&l->f, Pt(s.corner.x, my));
				nscr=scrpos(l->scroll, p0, p0+l->f.nchars, tot);
				y=nscr.origin.y;
			}else if(but==2){
				y=my;
				if(y>s.corner.y-2)
	...
	if(in){
		...
		rectf(l->f.b, r, F_XOR);
		if(but==1)
			outTsll(Torigin, t->tag, l->origin, (long)(my-s.origin.y)/fheight(l->f.font)+1);
		else if(but==2){
			if(tot>1024L*1024L)
