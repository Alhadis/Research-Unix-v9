...
char	*genmenu2c();
Text	*sweeptext();
/* enum Menu2 -- until we fix the C compile to treat enums like ints */
#define	Cut 0
#define	Paste 1
#define	Snarf 2
#define	Look 3
...
menu3hit()
{
	...
	case Reshape:
		cursswitch(&bullseye);
		buttons(Down);
		if(button3() && (l=flwhich(mouse.xy)) && getr(&r, 1))
...
sweeptext(new, tag)
{
	Rectangle r;
	register Text *t;
	if(getr(&r, 1) && (t=(Text *)alloc(sizeof(Text)))){
		current((Flayer *)0);
