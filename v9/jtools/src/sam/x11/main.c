	...
	char **argv;
{
	...
	for(;;){
		if ((P->state&KBD) && which && (lock || ((Text *)which->user1)->lock))
			wait(MOUSE|RCV);
		else
			wait(MOUSE|KBD|RCV);
		if (P->state&RCV) {
		...
		if(which && (P->state&KBD))
			type(which);
		if(lock<2){
			if(!button123() || !ptinrect(mouse.xy, Drect))
				continue;
	...
	Font *f;
{
	...
	if(nl){
		...
		flinit(nl, r, f);
		nl->origin=l->origin;
		flinsert(nl, (*l->textfn)(l, l->f.nchars), l->origin);
	...
	register Flayer *l;
{
	...
	while(!backspacing && (c=kbdchar())>0 && c!=ESC)
		switch(c&=0x7F){
		case '\027':
		case '\b':
			backspacing=1;
	...
	if(!lock && backspacing && l->f.p0>0){
		l->p1=a;
		l->p0=c=='\b'? a-1 : ctlw(&t->rasp, l->origin, a);
		cut(t, t->front, 0);
		if(typeaddr>=l->p0)
			typeaddr=l->p0;
	}else{
