	...
	int port;
{
	...
	strcpy(rhost, ipcpath(host, "tcp", "tcp.21"));
	s = ipcopen(rhost, "light hup");
	cin = fdopen(s, "r");
	cout = fdopen(s, "w");
...
initconn()
{
	...
	data = tcpcreat(portname, "heavy hup");
	if (data<0) {
		fprintf(stderr, "can't create data port\n");
		goto bad;
	}
	hostorderport = htons(fstotcp(portname));
	uname(&u);
	hostorderaddr = htonl(in_address(u.sysname));
