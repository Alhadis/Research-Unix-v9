/* $Header: ParseCmd.c,v 1.1 87/09/11 08:00:47 toddb Exp $ */
#ifndef lint
...
void XrmParseCommand(table, tableCount, prependName, argc, argv)
    ...
    char		**argv;		/* argument list (command line)	     */
{
	...
	for (i=0; (!foundOption) && i < tableCount; ++i) {
		...
		case XrmoptionNoArg:
		    ...
		    XrmPutResource(fullName, XrmQString, resourceValue);
		    break;
