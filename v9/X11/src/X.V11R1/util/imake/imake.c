...
char	*tmpMakefile    = "/usr/tmp/tm.XXXXXX";
char	*tmpImakefile    = "/usr/tmp/tim.XXXXXX";
	...
	FILE	*tmpfd;
{
	...
	if (! initialized) {
		...
		*end = '\0';
		unlink(fileno(tmpfd));
		close(fileno(tmpfd));
		fileno(tmpfd) = creat(tmpMakefile, 644);
		if (fileno(tmpfd) < 0)
			LogFatal("Cannot create temporary file %s.", tmpMakefile);
		initialized = TRUE;
