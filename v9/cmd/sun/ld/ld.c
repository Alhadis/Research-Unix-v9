#ifndef lint
static	char sccsid[] = "@(#)ld.c 1.1 86/02/03 SMI"; /* from UCB 4.9 83/07/01 */
#endif
...
delexit()
{
	...
	unlink(ofilename);
	if (delarg==0 && Aflag==0)
		chmod(aoutname, ofilemode);
	/*
	 * We have to insure that the last block of the data segment
	 * is allocated a full size page. If the underlying
	 * file system allocates frags that are smaller than pagesize,
	 * a full zero filled block needs to be allocated so 
...
setupout()
{
	...
	outb(&dout, outfilhdr.a_data);
	if (rflag) {
		outb(&trout, outfilhdr.a_trsize);
		outb(&drout, outfilhdr.a_drsize);
	}
	if (sflag==0 || xflag==0) {
		outb(&sout, outfilhdr.a_syms);
		wroff += sizeof (offset);
		outb(&strout, 0);
	}
}
...
finishout()
{
	...
	archdr.ar_name[0] = '\0';
	unlink(aoutname);
	if (link(ofilename, aoutname) < 0)
		error(1, "cannot move temp file %s to output", ofilename );
	delarg = errlev;
	delexit();
}
