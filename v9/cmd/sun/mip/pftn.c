...
#ifndef lint
static	char sccsid[] = "@(#)pftn.c 1.1 86/02/03 SMI";
#endif
...
# endif
	...
	if( class==MOU || class==MOS || class & FIELD ){/* make a new entry */
		...
		for( memp = &paramstk[paramno-1];
			/* while */ *memp>0 && STP(*memp)->sclass != STNAME
				&& STP(*memp)->sclass != UNAME;
			/* iterate */ --memp){ char * cname, * oname;
			if( STP(*memp)->sflags & SNONUNIQ ){int k;
				cname=p->sname;
				oname=STP(*memp)->sname;
...
noargs(){
	...
	for (i=0; i<paramno; ++i){
		if ( (j=paramstk[i]) <= 0) continue;
			uerror("%s declared as parameter to non-function", STP(j)->sname);
			STP(j)->slevel = 1;
			STP(j)->suse   = -lineno; /* shut lint up */
		}
	if (paramno){
		paramno  = 0;
		clearst( blevel );
...
# endif
		...
		else if( STP(id)->sclass & FIELD ){
			pstk->in_sz = - ( STP(id)->sclass & FLDSIZ );
			}
		else {
			pstk->in_sz = 0;
			}
		...
		if( ISARY(t) ){
			...
			id = dimtab[pstk->in_x];
			p = STP(id);
			if( pstk->in_x == -1 || p<=0 || p->sclass != MOS && !(p->sclass&FIELD) ) fatal("initialization with non-exist structure" );
			t = p->stype;
			d = p->dimoff;
			s = p->sizoff;
			off += p->offset;
...
fixtype( p, class ) register NODE *p; {
	...
	if( ISFTN(type) && DECREF(type) == FLOAT ){
		/* there are no FLOAT functions -- they're all DOUBLE */
#ifdef FLOATMATH
		if (FLOATMATH<=1)
#endif
		    type += DOUBLE-FLOAT;
...
fixclass( class, type ) TWORD type; {
	...
	if( ISFTN( type ) ){
		switch( class ) {
		default:
			uerror( "function %s has illegal storage class", curname );
		case AUTO:
			class = EXTERN;
		case EXTERN:
		case EXTDEF:
	...
	case REGISTER:
		if( blevel == 0 ) {
			uerror( "illegal register declaration %s", curname );
			return( AUTO );
		}
	...
	case ULABEL:
		if ( blevel < 2 )
			uerror( "illegal class for %s", curname );
		return( class );
...
struct symtab *
mknonuniq(idp) register idp; {/* locate a symbol table entry for */
	...
	char *p,*q;
        ...
        sp = STP( malloc (sizeof ( struct symtab )));
        sp->hashVal = STP(idp)->hashVal;
        sp->stype = TNULL;
	sp->sflags = SNONUNIQ | SMOS;
        sp->next = stab[STP(idp)->hashVal];
        stab[STP(idp)->hashVal] = sp;
	p = sp->sname;
	q = STP(idp)->sname; /* old entry name */
	sp->sname = STP(idp)->sname;
# ifndef BUG1
	if( ddebug ){
...
hide( p ) register struct symtab *p; {
	register struct symtab *q;
        q = STP( malloc (sizeof ( struct symtab )));
	movestab( q, p );
	p->sflags |= SHIDDEN;
	...
	if (blevel==2 && p->slevel==1) werror( "declaration of %s hides parameter", p->sname );
# ifndef BUG1
