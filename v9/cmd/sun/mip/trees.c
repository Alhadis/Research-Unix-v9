#ifndef lint
static	char sccsid[] = "@(#)trees.c 1.1 86/02/03 SMI";
#endif
...
# endif
	...
	if( actions&LVAL ){ /* check left descendent */
		if( notlval(p->in.left) ) {
			TYPEO(p->in.left->in.type,  "illegal lhs of assignment operator" );
			}
		}
	...
	if (actions & ELINT) p->in.left = makety( p->in.left, INT, 0, INT );
#endif
	...
	if( actions & OTHER ){
		...
		case STREF:
			...
			i = r->tn.rval;
			if( i<=0 || ((sp= STP(i))->sclass != MOS && sp->sclass != MOU && !(sp->sclass&FIELD)) ){
				TYPEO( sp->stype, "member of structure or union required" );
				}else
			/* if this name is non-unique, find right one */
			if( STP(i)->sflags & SNONUNIQ &&
...
# endif
...
#ifndef FLEXNAMES
					werror( "illegal member use: %.8s", STP(i)->sname );
#else
					werror( "illegal member use: %s", STP(i)->sname );
#endif
					}
				}
		...
		case UNARY MUL:
			...
			if( !ISPTR(l->in.type))TYPEO(l->in.type,"illegal indirection");
...
# endif
		...
		case CAST:
...

			{
				...
				if( l->fn.csiz != r->fn.csiz ) TYPEO2( l->in.type, r->fn.type, "assignment of different structures" );
		...
		case COLON:
			...
			if( l->fn.csiz != r->fn.csiz ) TYPEO2( l->in.type, r->fn.type, "type clash in conditional" );
			break;

		case CALL:
			p->in.right = r = strargs( p->in.right );
		case UNARY CALL:
			if( !ISPTR(l->in.type)) TYPEO(l->in.type,"illegal function");
			p->in.type = DECREF(l->in.type);
			if( !ISFTN(p->in.type)) TYPEO(p->in.type,"illegal function");
			p->in.type = DECREF( p->in.type );
			p->fn.cdim = l->fn.cdim;
			p->fn.csiz = l->fn.csiz;
			if( l->in.op == UNARY AND && l->in.left->in.op == NAME &&
				l->in.left->tn.rval > 0 && l->in.left->tn.rval != NONAME &&
				( (i=STP(l->in.left->tn.rval)->sclass) == FORTRAN || i==UFORTRAN ) ){
	...
	if( (k = j) < 0 ) uerror( "undefined structure or union" );
		...
		for( ; (kk = dimtab[k] ) > 0; ++k ){
			...
			case UNIONTY:
				...
				if( hflag && chkstr( i, dimtab[STP(kk)->sizoff+1], STP(kk)->stype ) ){
					if( STP(kk)->sname[0] == '$' ) return(0);  /* $FAKE */
					werror(
#ifndef FLEXNAMES
...
chkpun(p) register NODE *p; {
	...
	if (BTYPE(t1) == TERROR || BTYPE(t1) == TERROR) return;
...
psize( p ) NODE *p; {
	...
	if( !ISPTR(p->in.type) ){
		TYPEO(p->in.type,  "pointer required");
		return( SZINT );
		}
...
ptmatch(p)  register NODE *p; {
	...
	default:  /* must work harder: relationals or comparisons */
		...
		switch (o) {
			/* relationals should be "unsigned" */
			case GE:
			case GT:
			case LE:
			case LT:
