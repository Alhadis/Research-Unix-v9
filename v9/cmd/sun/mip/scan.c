#ifndef lint
static	char sccsid[] = "@(#)scan.c 1.1 86/02/03 SMI"; /* from UCB X.X XX/XX/XX */
#endif
...
mainp1( argc, argv ) int argc; char *argv[]; {  /* control multiple files */
	...
	int fdef = 0;
	char *release = "PCC/XXX SUN 4.1c bsd";
...
yylex(){
	for(;;){
		...
		xreset();
		switch( (p=lxcp[(lxchar=xgetchar())+1])->lxact ){
...
# endif
		...
		case A_EQ:
			...
			warn:
				if( lxmask[ (lxchar=xgetchar())+1] & (LEXLET|LEXDIG|LEXDOT) ){
