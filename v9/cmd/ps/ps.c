...
int argc; char **argv;
{
	...
	if ((devlist = getdir("/dev", devlist)) == 0)
		error("can't getdir /dev");
	if ((devlist = getdir("/dev/dk", devlist)) == 0)
		error("can't getdir /dev/dk");
	if ((devlist = getdir("/dev/pt", devlist)) == 0)
		error("can't getdir /dev/pt");
