...
char *s; register struct proc *pp; register struct user *up;
{
	...
	for (sp=selbeg; sp<selend; sp++) {
		if (sp->flag & SELTTY) {
			if (error = getuarea(fd, pp, up))
				Error(error);
			if (strncmp(sp->id, gettty(up->u_ttyino), 5))
				continue;
...
{
	extern errno;
	struct { char block[ctob(btoc(sizeof(struct user)))];} block, *bp;
	...
	bp = &block;
	if (Sread(drum, pp->p_swaddr*NBPG/ctod(1), bp)) {
		*up = *(struct user *)&block;
		return 0;
	}
