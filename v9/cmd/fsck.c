...
#define SMALL	0	/* 1k file systems */
#define NDIRECT(x)	(BSIZE(x)/sizeof(struct direct))
#define SPERB(x)	(BSIZE(x)/sizeof(short))
...
struct bufarea {
	...
	daddr_t	b_bno;
	union {
		char	b_buf[BSIZE(BIG)];		/* buffer space */
		short	b_lnks[SPERB(BIG)];		/* link counts */
		daddr_t	b_indir[NINDIR(BIG)];	/* indirect block */
		struct filsys b_fs;		/* super block */
		struct fblk b_fb;		/* free block */
		struct dinode b_dinode[INOPB(BIG)];	/* inode block */
		DIRECT b_dir[NDIRECT(BIG)];		/* directory */
	} b_un;
	char	b_dirty;
};
...
#endif
#ifdef mc68000
#define	MAXDATA ((MEMSIZE)500*1024)
#endif
...
char	big;			/* it is a 4k file system */
	...
	char	*name;
{
  ...
  retry:
	...
	big = SMALL;		/* 1k blocks by default */
	if (stat_block.st_mode & S_IFBLK){
		if(BITFS(stat_block.st_rdev))
			big = BIG;		/* a 4k file system */
		raw = rawname(name);
...
char *dev;
{
	...
	if(rawflg) {	/* shouldn't be set for 4k system */
		if(inoblk.b_dirty)
			bwrite(&dfile,membase,startib,(int)niblk*BSIZE(big));
		inoblk.b_dirty = 0;
		if(poolhead) {
			clear(membase,niblk*BSIZE(BIG));
			for(bp1 = poolhead;bp1->b_next;bp1 = bp1->b_next);
			bp2 = &((BUFAREA *)membase)[(niblk*BSIZE(BIG))/sizeof(BUFAREA)];
	...
	if(sflag || (csflag && rplyflag == 0)) {
		...
		if(freemap)
			...
			for(blk = 0; blk < fmapblk; blk++) {
				...
				copy(bp1->b_un.b_buf,bp2->b_un.b_buf,BSIZE(big));
				dirty(bp2);
			}
		}
		badblk = dupblk = 0;
		...
		if(fixfree == 0) {
			...
			else if(big == SMALL && n_free != superblk.s_tfree) {
...
register ilevel;
{
	...
	if(getblk(&ib,blk) == NULL)
		return(SKIP);
	ilevel--;
	for(ap = ib.b_un.b_indir; ap < &ib.b_un.b_indir[NINDIR(big)]; ap++) {
		if(*ap) {
			if(ilevel > 0) {
...
daddr_t blk;
{
	...
	for(dirp = dirblk.b_dir; dirp < &dirblk.b_dir[NDIRECT(big)] &&
		filsize > 0; dirp++, filsize -= sizeof(DIRECT)) {
		if(getblk(&fileblk,blk) == NULL) {
			filsize -= (&dirblk.b_dir[NDIRECT(big)]-dirp)*
				sizeof(DIRECT);
			return(SKIP);
		}
...
char *dev;
{
	...
	rawflg = 0;
	big = SMALL;	/* does this undo the work in blockcheck? */
	if((statarea.st_mode & S_IFMT) == S_IFBLK) {
		if(BITFS(statarea.st_rdev))
			big = BIG;
	}
	...
	imax = ((ino_t)superblk.s_isize - (SUPERB+1)) * INOPB(big);
	fmin = (daddr_t)superblk.s_isize;	/* first data blk num */
	fmax = superblk.s_fsize;		/* first invalid blk num */
	if(fmin >= fmax || 
		(imax/INOPB(big)) != ((ino_t)superblk.s_isize-(SUPERB+1))) {
	...
	mbase = membase;
	if(rawflg) {
		if(msize < (MEMSIZE)(NINOBLK*BSIZE(big)) + 2*sizeof(BUFAREA))
			rawflg = 0;
		else {
			msize -= (MEMSIZE)NINOBLK*BSIZE(big);
			mbase += (MEMSIZE)NINOBLK*BSIZE(big);
			niblk = NINOBLK;
			startib = fmax;
		}
	}
	...
	if((off_t)msize < totsz) {
		bmapsz = roundup(bmapsz,BSIZE(big));
		smapsz = roundup(smapsz,BSIZE(big));
		lncntsz = roundup(lncntsz,BSIZE(big));
		nscrblk = (bmapsz+smapsz+lncntsz)>>BSHIFT(big);
		if(tflag == 0) {
		...
		lncntp = (short *) NULL;
		smapblk = bmapsz / BSIZE(big);
		lncntblk = smapblk + smapsz / BSIZE(big);
		fmapblk = smapblk;
	}
	else {
		if(rawflg && (off_t)msize > totsz+BSIZE(big)) {
			niblk += (unsigned)((off_t)msize-totsz)>>BSHIFT(big);
			if(niblk > MAXRAW)
				niblk = MAXRAW;
			msize = memsize - (niblk*BSIZE(big));
			mbase = membase + (niblk*BSIZE(big));
		}
		poolhead = NULL;
		blkmap = mbase;
...
ginode()
{
	...
	if(rawflg) {
		...
		if(iblk < startib || iblk >= startib+niblk) {
			if(inoblk.b_dirty)
				bwrite(&dfile,mbase,startib,(int)niblk*BSIZE(big));
			inoblk.b_dirty = 0;
			if(bread(&dfile,mbase,iblk,(int)niblk*BSIZE(big)) == NO) {
				startib = fmax;
				return(NULL);
			}
			startib = iblk;
		}
		dp = (DINODE *)&mbase[(unsigned)((iblk-startib)<<BSHIFT(big))];
	}
	else if(getblk(&inoblk,iblk) != NULL)
		dp = inoblk.b_un.b_dinode;
	else
		return(NULL);
	return(dp + itoo(big, inum));
}
...
dostate(s,flg)
{
	...
	else if((bp = getblk((BUFAREA *)NULL,
			(daddr_t)(smapblk+(byte/BSIZE(big))))) == NULL)
		errexit("Fatal I/O error\n");
	else
		p = &bp->b_un.b_buf[byte%BSIZE(big)];
	switch(flg) {
		case 0:
			*p &= ~(SMASK<<(shift));
			*p |= s<<(shift);
...
daddr_t blk;
{
	...
	else if((bp = getblk((BUFAREA *)NULL,blk+(byte>>BSHIFT(big)))) == NULL)
		errexit("Fatal I/O error\n");
	else
		p = &bp->b_un.b_buf[(unsigned)(byte&BMASK(big))];
	switch(flg&03) {
		case 0:
			*p |= n;
			break;
...
short val;
{
	...
	else if((bp = getblk((BUFAREA *)NULL,(daddr_t)(lncntblk+(inum/SPERB(big))))) == NULL)
		errexit("Fatal I/O error\n");
	else
		sp = &bp->b_un.b_lnks[inum%SPERB(big)];
	switch(flg) {
		case 0:
			*sp = val;
			break;
...
register BUFAREA *bp;
{
	...
	if(bp->b_bno == blk)
		return(bp);
	flush(fcp,bp);
	if(bread(fcp,bp->b_un.b_buf,blk,BSIZE(big)) != NO) {
		bp->b_bno = blk;
		return(bp);
	}
	bp->b_bno = (daddr_t)-1;
...
{
	if(bp->b_dirty) {
		bwrite(fcp,bp->b_un.b_buf,bp->b_bno,BSIZE(big));
	}
	bp->b_dirty = 0;
}
...
freechk()
{
	...
	if(big == BIG) {
		for(i = superblk.s_isize; i < superblk.s_fsize; i++) {
			j = i - superblk.s_isize;
			if(!getfmap(i)) {
				superblk.s_bfree[j>>5] |= (1 << (j&31));
				n_free++;
			}
			else
				superblk.s_bfree[j>>5] &= ~(1 << (j&31));
...
linkup()
{
	...
	if(dp->di_size & BMASK(big)) {
		dp->di_size = roundup(dp->di_size,BSIZE(big));
		inodirty();
	}
	filsize = dp->di_size;
	inum = orphan;
...
char *buf;
{
	if(lseek(fcp->rfdes,blk<<BSHIFT(big),0) < 0)
		rwerr("SEEK",blk);
...
char *buf;
{
	if(fcp->wfdes < 0)
		return(NO);
	if(lseek(fcp->wfdes,blk<<BSHIFT(big),0) < 0)
		rwerr("SEEK",blk);
