...
# define SBSIZE 96000		/* std = 12000, wnj aug 1979 */
char	sbf[SBSIZE];
char	*savch	= sbf;
...
#ifdef	MTIME
STATIC	long	mtimes[MAXINC];
STATIC	int	mflag;	/* modify date in # 12 foo.c lines */
#endif
...
# define symsiz 500
# else
# define symsiz 5000		/* 1000 won't do all sys/h (pjw) */
# endif
...
/* BB */ STATIC struct symtab *claloc;
STATIC	int	trulvl;
STATIC	int	flslvl;
...
/* BB */
	} else if (np==claloc) {/* class */
#define CLASS 27
		if (exfail == 0) exfail = CLASS;
