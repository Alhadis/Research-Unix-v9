...
int	term, chksum, wflag, recno, first, linkerrok, Lflag;
int	freemem = 1;
int	nblock = DBLOCK;
...
usage()
{
	fprintf(stderr, "tar: usage  tar -{txru}[cvfblLm] [tapefile] [blocksize] file1 file2...\n");
	done(1);
}
...
char *shortname;
{
	...
	if (Lflag && lstat (shortname, &stbuf) == 0 &&
	    (stbuf.st_mode & S_IFMT) == S_IFLNK) {
		char lname[NAMSIZ+1];
		int len = readlink (shortname, lname, NAMSIZ+1);
		if (len < 0) {
			perror (longname);
			return;
		} else if (len > NAMSIZ) {
			fprintf (stderr,
			    "tar: %s symlink name too long, ignored\n", longname);
			return;
		}
		lname[len] = '\0';
		...
		cp2 = longname;
		for (cp = dblock.dbuf.name, i=0;
			(*cp++ = *cp2++) && i < NAMSIZ; i++);
		if (i >= NAMSIZ) {
			fprintf(stderr, "%s: file name too long\n", longname);
			return;
		}
		strcpy(dblock.dbuf.linkname, lname);
		dblock.dbuf.linkflag = 's';
		sprintf(dblock.dbuf.chksum, "%6o", checksum());
		writetape( (char *) &dblock);
		if (vflag) {
			fprintf(stderr, "a %s ", longname);
			fprintf(stderr, "symlink to %s\n", lname);
		}
		return;
	}
	...
	if ((stbuf.st_mode & S_IFMT) == S_IFDIR) {
		...
		*++cp = 0  ;
		...
		close(infile);
		dp = opendir(".");
		while ((dbuf = readdir(dp)) != NULL && !term) {
			if (dbuf->d_ino == 0) {
				continue;
			}
...
gotit:
		...
		if (dblock.dbuf.linkflag == 's') {
			...
			if (symlink(dblock.dbuf.linkname, dblock.dbuf.name) < 0) {
				fprintf(stderr, "%s: cannot symlink\n", dblock.dbuf.name);
				continue;
			}
			if (vflag)
				fprintf(stderr, "%s -> %s\n",
...
/*
 * backup over last `tape' block
 * hack: if it's a raw magtape, do the ioctl
 */
backtape()
{
	static int isatape = 1;
...
char *longptr, *shortptr, *malloc(), *endbit, *strrchr();
	...
	sprintf(shortptr+MAXFILENAME-4,"..");
...
char *original;
{
  int length;
  char newname[100];
  register char *inend, *outptr=newname, *instart=original,
		*outstart;
 int changed = 0;
  ...
  if(changed){
	if(ntoolong == 0) {
		longnamefd = fopen("longnamelist","w");
		if(longnamefd == NULL){
			fprintf(stderr,
				"can't create longnamelist file\n");
			exit(1);
		}
		fprintf(stderr,"check out file 'longnamelist'\n");
	}
	printf("%s changed to %s\n",original,newname);
	fprintf(longnamefd,"%s\t%s\n",original, newname);
	fflush(longnamefd);
