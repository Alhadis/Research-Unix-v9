static char *sccsid = "@(#)mkfs.c	4.2 (Berkeley) 4/20/81";

/*
 * Make a file system prototype.
 * usage: mkfs filsys proto/size [ m n ]
 */
#define	NIPB	(BSIZE(0)/sizeof(struct dinode))
#define	NDIRECT	(BSIZE(0)/sizeof(struct direct))
#define	LADDR	10
#define	MAXFN	1000
#define MAXISIZE (65536-NIPB)
#ifndef STANDALONE
...
char *bf;
{
	...
	lseek(fso, bno*BSIZE(0), 0);
	n = write(fso, bf, BSIZE(0));
	if(n != BSIZE(0)) {
