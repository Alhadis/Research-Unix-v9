	...
	struct proc *p;
{
	...
	*(int *)&proto = PG_URKR;
	if (xp->x_flag & XLOAD) {
		proto.pg_fod = 1;
		((struct fpte *)&proto)->pg_source = PG_FZERO - PG_FMIN;
	}
	...
	if (xp->x_flag & XLOAD)
		...
		while (num > 0) {
			...
			blkno += fsbtodb(swapdev, 1);
			pte += BUFSIZE/sizeof (struct pte);
			num -= BUFSIZE;
		}
...
#ifdef vax
		if(newu->u_pcb.pcb_ssp != -1) tf |= 1;
		if(newu->u_pcb.pcb_esp != -1) tf |= 2;
#endif
		if(newu->u_tsize != p->p_tsize) tf |= 4;
		if(newu->u_dsize != p->p_dsize) tf |= 8;
		if(newu->u_ssize != p->p_ssize) tf |= 16;
		if(newu->u_procp != p) tf |= 32;
		if(tf) {
			printf("vgetu %d ", tf);
			panic("vgetu");
		}
...
top:
	...
	p2 = p1 + change;
	while (p1 < p2) {
		mapin(p1, btop(kmxtob(i)), p1->pg_pfnum, 1, PG_V|PG_KW);
