	...
	struct text *xp;
{
	...
	if (xp->x_flag & XPAGI) {
		xp->x_ptdaddr =
		    rmalloc(swapmap, ctod(clrnd(ctopt(xp->x_size))));
		if (xp->x_ptdaddr == 0) {
	...
	int ts;
{
	...
	if ((xp->x_flag&XPAGI) && xp->x_ptdaddr) {
		rmfree(swapmap, ctod(clrnd(ctopt(xp->x_size))), xp->x_ptdaddr);
		xp->x_ptdaddr = 0;
	}
}
	...
	struct dmap *dmp;
{
	...
	if (type == CTEXT) {
		while (size > 0) {
			...
			swap(p, p->p_textp->x_daddr[base/DMTEXT] + base%DMTEXT,
			    ptob(tptov(p, dtoc(base))), dtob(db.db_size),
			    B_WRITE, 0, swapdev, 0);
			pte = tptopte(p, dtoc(base));
			p->p_textp->x_rssize -=
			    vmemfree(pte, dtoc(db.db_size));
			base += db.db_size;
			size -= db.db_size;
		}
		return;
	...
	do {
		...
		v = type==CSTACK ?
		    ...
		    dptov(p, dtoc(base));
		swap(p, db.db_base, ptob(v), dtob(db.db_size),
		    B_WRITE, 0, swapdev, 0);
		pte = type==CSTACK ?
		    sptopte(p, dtoc(base+db.db_size)-1) :
		    dptopte(p, dtoc(base));
		p->p_rssize -= vmemfree(pte, dtoc(db.db_size));
		base += db.db_size;
		size -= db.db_size;
	} while (size != 0);
}
