...
creat()
{
	...
	nmarg = nilargnamei;
	nmarg.flag = NI_CREAT;
	nmarg.mode = uap->fmode & 07777 & ~u.u_cmask;
	nmarg.done = 0;
	ip = namei(uchar, &nmarg, 1);
	if(ip == NULL) 
		return;
	open1(ip, FWRITE, nmarg.done == 0? 1: 2);
}
...
close()
{
	...
	if (u.u_pofile[uap->fdes] & MMAPPED)
		munmapfd(uap->fdes);
...
link()
{
	...
	uap = (struct a *)u.u_ap;
	ip = namei(uchar, (struct argnamei *)NULL, 0);   /* well, this routine is doomed anyhow */
	if(ip == NULL)
		return;
...
mknod()
{
	...
	if(suser()) {
		...
		nmarg.mode = uap->fmode & ~u.u_cmask;
