...
float	avenrun[3];		/* load average, of runnable procs */
...
#define	swappable(p) (((p)->p_flag & \
	(SSYS|SLOCK|SULOCK|SLOAD|SPAGE|SKEEP|SWEXIT|SPHYSIO|SPROCIO)) == SLOAD)
...
loop:
	...
	for (rp = proc; rp < procNPROC; rp++)
	    switch(rp->p_flag&SPROCIO ? SRUN : rp->p_stat) {

	case SRUN:
		if ((rp->p_flag&SLOAD) == 0) {
			rppri = rp->p_flag&SPROCIO ? 99999 : rp->p_time -
...
vmmeter()
{
	...
	if (time % 5 == 0) {	/* every fifth time into vmmeter */
		vmtotal();
	...
	int n;
{
	...
	for (i = 0; i < 3; i++)
		avg[i] = cexp[i] * avg[i] + n * ((float)1.0 - cexp[i]);
}
