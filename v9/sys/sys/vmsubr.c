	...
	size_t count;
{
	...
	if(ip->i_fstyp)
		panic("vinifod, fstyp");
	if(fileno < PG_FMIN)
		panic("vinifod, fileno");
	while (count > 0) {
		if (bstart < NADDR - 3) {
			pte->pg_fod = 1;
			pte->pg_source = fileno - PG_FMIN;
			pte->pg_blkno = ip ?
				fsbtodb(ip->i_dev,ip->i_un.i_addr[bstart]) : 0;
			if (pte->pg_blkno == 0) {
				pte->pg_source = PG_FZERO - PG_FMIN;
				pte->pg_blkno = 0;
				cnt.v_nzfod += CLSIZE;
			} else if (fileno == PG_FTEXT)
				cnt.v_nexfod += CLSIZE;
			else
				panic("vinifod, vrpages set");
			for (j = 1; j < CLSIZE; j++)
				pte[j] = pte[0];
			pte += CLSIZE;
			bstart++;
			...
			if (bp) {
				...
				do {
					pte->pg_fod = 1;
					pte->pg_blkno = fsbtodb(ip->i_dev,*pp);
					pp++;
					if (pte->pg_blkno) {
						pte->pg_source = fileno - PG_FMIN;
						if (fileno == PG_FTEXT)
