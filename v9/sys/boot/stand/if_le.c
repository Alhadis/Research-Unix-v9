...
struct lance_softc {
	...
	struct le_md		*es_rmdp[2];	/* Receive Desc Ring ptrs */
	...
	struct saioreq *sip;
{
	...
	bzero((caddr_t)es->es_rmdp[0], 2 * sizeof (struct le_md));
	...
	install_buf_in_rmd(es->es_rbuf[0], es->es_rmdp[0]);
	install_buf_in_rmd(es->es_rbuf[1], es->es_rmdp[1]);
