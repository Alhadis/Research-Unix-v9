...
loop:
	...
	if (blkno < dbtofsb(0,bp->b_blkno)) {
		...
		tc->c_repcnt = dbtofsb(0,bp->b_blkno) - blkno;
	} else {
		bp->b_command = TS_SREV;
		tc->c_repcnt = blkno - dbtofsb(0,bp->b_blkno);
	}
dobpcmd:
	/*
...
ignoreerr:
	...
	case SSEEK:
		sc->sc_blkno = dbtofsb(0,bp->b_blkno);
		goto opcont;
	...
	register struct buf *bp;
{
	...
	if (bp == &ctsbuf[TSUNIT(bp->b_dev)]) {
		if (sc->sc_blkno > dbtofsb(0,bp->b_blkno)) {
			/* reversing */
			sc->sc_nxrec = dbtofsb(0,bp->b_blkno) - sc->sc_sts.s_rbpcr;
			sc->sc_blkno = sc->sc_nxrec;
		} else {
			/* spacing forward */
			sc->sc_blkno = dbtofsb(0,bp->b_blkno) + sc->sc_sts.s_rbpcr;
