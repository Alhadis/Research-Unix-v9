...
daddr_t blkno;
{
	...
	u.u_procp->p_lnode->kl_cost += shconsts.sc_bio;
	shconsts.sc_bioc++;
	iowait(bp);
	return(bp);
}
...
daddr_t blkno;
{
	...
	bp->b_flags = B_BUSY | (u.u_procp->p_usrpri << B_PRISHIFT);
...
loop:
	...
	bp->b_flags = B_BUSY | B_INVAL | (u.u_procp->p_usrpri << B_PRISHIFT);
	bp->b_back->b_forw = bp->b_forw;
	bp->b_forw->b_back = bp->b_back;
	bp->b_forw = dp->b_forw;
	bp->b_back = dp;
...
unsigned (*mincnt)();
{
	...
	while (u.u_count != 0) {
		bp->b_flags = B_BUSY | B_PHYS | rw | (u.u_procp->p_usrpri << B_PRISHIFT);
		bp->b_dev = dev;
