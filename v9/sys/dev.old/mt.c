...
struct	mu_softc {
	...
	int	sc_erreg;		/* copy of mter or mtner */
	int	sc_dsreg;		/* copy of mtds */
	...
	int flag;
{
	...
	sc->sc_nxrec =  ((flag & (FREAD | FWRITE)) == FWRITE)
		      ? (daddr_t)0
		      : (daddr_t)INF;
	sc->sc_flags = 0;
}
	...
	register struct mba_device *mi;
{
	...
	if (   (bp == &rmtbuf[MTUNIT(bp->b_dev)])
	    || ((blkno = sc->sc_blkno) == dbtofsb(0, bp->b_blkno)) ) {
	...
	int mbsr;
{
	...
	case MTER_RDOPP:
		...
		/* The byte count in the controller may be larger than	*/
		/* the size of the buffer, so don't bother to re-try if	*/
		/* it will eventually be an error anyway.  If the	*/
		/* controller returns a byte count of zero, change it	*/
		/* to 1, since zero encodes 65536, which isn't quite	*/
		/* what we had in mind.  If the count is reasonable,	*/
		/* set bcount to the negative of the controller's	*/
		/* version of the byte count so that the start address	*/
		/* for the transfer is set up correctly.		*/

		if (MASKREG(mtaddr->mtbc) <= bp->b_resid) {
			if (mt_do_readrev) {
	...
	register struct mba_device *mi;
{
	...
	case MTER_LEOT:
		...
		} else if (sc->sc_blkno > dbtofsb(0, bp->b_blkno)) {
			sc->sc_nxrec = dbtofsb(0, bp->b_blkno) + fc - 1;
