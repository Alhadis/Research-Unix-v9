...
#define	p_ignsig p_siga0	/* ignored signal mask */
	...
	short	p_ppid;		/* process id of parent */
 	KL_p	p_lnode;	/* per-user scheduling */
 	float	p_sharepri;	/* priority based on p_nice and p_lnode->kl.l_usage */
	...
	u_short	p_clktim;	/* time to alarm clock signal */
	u_short	p_tsleep;	/* time to tsleep */
	...
	struct	inode *p_trace;	/* inode for new process tracing stuff */
#ifdef sun
...
#define	SDETACH	0x00080000	/* detached inherited by init */
#define	SNUSIG	0x00100000	/* using new signal mechanism */
...
#define	SPTECHG	0x10000000	/* pte's for process have changed */
...
struct	xproc
{
	...
	struct	pte *xp_addr;
	char	xp_usrpri;	/* user-priority based on p_cpu and p_nice */
	...
	short	xp_ppid;	/* process id of parent */
 	KL_p	xp_lnode;	/* per-user scheduling */
 	float	xp_sharepri;	/* priority based on p_nice and p_lnode->kl.l_usage */
